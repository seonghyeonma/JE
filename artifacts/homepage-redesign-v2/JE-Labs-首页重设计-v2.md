...
- 背景图改为**全浏览器宽度 full-bleed**，不再收在内容框里。Header 和文案锁在 1440px 内容网格内，宽屏展示更舒展，同时避免文字随超宽屏漂向两端。
...
- **原来的 4 个聚合数字已移入 Work 区块标题下方**（见第 6 节）
...
**两家都证明了同一件事：数字必须和成果叙事连在一起，而不是漂浮在首页前部。**

三条理由：

1. **聚合数字属于 Work，而不是 Proof。** 它们必须跟在 `What the system produced` 之后，直接回答 JE Labs 总体做出了什么。
2. **原来的顺序是"数字 → 方法论 → 数字"**，中间夹一个方法论，把刚建立的叙事流（身份 → 怎么做 → 做出了什么）打断了。
3. Work 内部采用**总览再举证**：先用一张连续数据流和四项累计结果建立规模感，再用 PublicAI、MOSS AI、SURF AI 三个案例说明这些结果如何发生。

第二屏继续只承担客户信任，不提前展示数字；访客经过方法论后进入 Work，才一次性看到总体规模与具体案例。这样数字出现得稍晚，但语义更完整。
...
![案例](img/qa/work-cards-desktop-v6.png)

**改成什么**

V6 保留 UNRVLD 式紧凑横向浏览节奏，不靠扩大卡片间距解决粘连。问题改在单卡内部处理：每张案例成为一个有明确边界的完整叙事单元，先说明客户是谁，再说明 JE Labs 带来的改变，最后用独立结果面板承载三项证据。

```
[ 16:9 案例图，贴合卡片上沿 ]
[ 客户名 ]
[ 公司是什么：一句准确介绍 ]
[ JE Labs 带来的改变：一句结果叙事 ]
[ 结果面板：数字 | 结果说明 ]
[           数字 | 结果说明 ]
[           数字 | 结果说明 ]
[ engagement scope ]
...
桌面端仍然同时显示两张完整案例和第三张的一部分，卡片间距保持 `24px`；移动端保持 `88vw` scroll-snap 和 `16px` 间距。卡片本身使用 `1px` 中性边框、`12px` 圆角和深一层底色，图片、介绍、改变与指标被收进同一边界，因而相邻案例再近也不会把数据读成一组。

PublicAI、MOSS AI 与 SURF AI 都补充公司定位。PublicAI 写明其去中心化 AI 数据基础设施与全球贡献者网络；MOSS AI 写明其用自然语言构建和运行 AI 交易代理的平台定位；SURF AI 写明其面向加密和数字资产研究的 AI 情报平台定位。三句分别依据品牌官网或官方公司资料压缩，不使用泛泛的 “AI company”。

案例编号贴片和 chip 继续删除。scope 保持卡片底部的普通文本，hover 只让当前图片轻微放大和提亮，不压暗其他案例。
...
1. **数字继续用主色 + 等宽字**。每项成果改成两列纵向行：左侧数字，右侧结果说明。三个结果收进同一内嵌面板，并只在行之间使用细分隔线。
2. **PublicAI 的第三格 `Expanded` 不是数字**，所以不给它数字的待遇：换成正文字体、25px、中性灰。三个格子里有一个是定性结果，如果排版一模一样会显得在凑数。
3. **图片是主视觉，不在图片上再叠序号和标签**。客户名、scope 和成果都放在图片外，内容层级更接近成熟作品站。
4. **三项成果都与对应案例绑定**。整张卡片形成完整边界，内部结果面板形成第二层边界，避免数字在横向轨道里跨卡串联。

**区块开场：总体成绩概览**

聚合数据紧跟 `Work / What the system produced` 标题出现，再进入 PublicAI、MOSS AI、SURF AI 三个案例。顺序是“总成绩 → 具体证据”，因此图、数字和案例都属于同一个 Work 区块。

视觉上不再使用圆角四格卡片，也不再增加 `Impact at scale` 这层标签和独立章节标题。`Our track record to date` 只作为 Work 标题下的累计范围说明，下面直接是一张从收束到扩张的薄荷色数据流主视觉，以及 `100+` / `100M+` / `1,000+` / `15` 单行四列数据。删除模块外框和上下章节线，让它与 Work 标题形成同一个开场组合。

这个方向结合了两类成熟做法：Attentive 用大幅视觉与少量数字建立权威感；Stripe 用连续的数据主视觉把几个指标组织成一个系统。JE Labs 不复制它们的配色或具体波形，而是将策略、创作者、媒体和市场抽象成同一条薄荷色网络流，继续遵守“一种强调色、无发光、无阴影”的品牌规则。

数据流增加了有明确方向的动态层：信号从左侧进入，在画面中心汇聚，随后向右分叉并逐步增加线宽、亮度与路径数量，表达“经过 JE Labs 的整合后，传播路径和市场曝光被放大”。动画只在区块进入视口时运行，并在 `prefers-reduced-motion` 下停用，保留静态主视觉。

四项聚合数字全部使用静态品牌绿 `#06F5B7`。删除文字渐变和循环动画，只保留数据流主视觉本身的方向性运动。这样动态负责解释传播放大，数字负责稳定承载结果，两者职责不再重叠。

视觉上必须和案例数字**分层**：

| | 字号 | 颜色 | 含义 |
|---|---|---|---|
| 案例数字 | 33-40px 等宽 | **主色** | 具体成果，是这一屏的主角 |
| 聚合数字 | 62px 等宽 | **静态品牌绿** | 总量，是 Work 的开场概览 |

聚合数字只靠字号建立层级，不再靠文字动画抢注意力。

**案例版式研究**：除用户给出的 Attentive 与 UNRVLD 外，本轮还查看了 Stripe、Work & Co、NoGood、Instrument、Vercel 和 Google Cloud 的官方案例页。

| 参考 | 采用的判断 |
|---|---|
| [Attentive](https://www.attentive.com/case-studies) | 单个旗舰案例用大幅图片和少量巨大数字建立深度 |
| [UNRVLD](https://www.unrvld.com/work) | 媒体尺度优先，项目之间需要明显的浏览节奏 |
| [Stripe](https://stripe.com/customers) | 成果数字必须直接附着在客户案例上 |
| [Work & Co](https://work.co/work) | 用图片和项目名说话，减少 UI 式标签 |
| [NoGood](https://nogood.io/results/) | 让增长数字成为案例中的主要证据 |
| [Instrument](https://www.instrument.com/home) | 真实项目媒体使用不同尺度，不做整齐划一的模板网格 |
| [Vercel](https://vercel.com/customers) | 标题直接写业务结果，减少抽象形容词 |
| [Google Cloud](https://cloud.google.com/customers) | 先建立主案例，再用结果集合扩展可信度 |

最终没有照搬 UNRVLD 的横向轮播，因为 JE Labs 目前只有三个案例。强行轮播会隐藏内容，也会让访客多一次操作。当前采用“一个旗舰 + 两个错层次级案例”，更适合现有内容量。
...
![结尾](img/qa/contact-cta-desktop-v7.png)
...
- **结尾 CTA 改为非对称双栏**：左侧用大号收束句 `Tell us the market you have not won yet` 回扣首屏，右侧使用中性深灰联系面板。薄荷绿只保留在 2px 顶边、主按钮和回复时效，不再铺满整张卡片。
- 联系面板标题 `Start a conversation` 在桌面和移动端都保持单行；Email 是唯一实心主操作，Telegram 降为描边次操作。
- **真页脚**：4 栏（品牌简介 / Company / Answers / Elsewhere）。**Answers 一栏就是原来的 FAQ**，5 个问题变成 5 个页脚链接，`schema.org/FAQPage` 标记原样保留在折叠区里。

**为什么**

- 结尾 CTA 需要比普通内容区更强，但整块高饱和绿色会抢走左侧主张并切断深色页面的连续性。中性面板保留了清晰边界，单个绿色按钮仍能承担转化焦点。
...
| Work 案例布局 | 旗舰案例与两个次级案例全部改为单栏，错层取消，图片常驻 |
...
| 优先级    | 素材            | 规格                                | 用在哪                  |
| ------ | ------------- | --------------------------------- | -------------------- |
| **P0** | 3 张案例图        | 1600×1000，每客户一张                   | Work 案例主视觉         |
...
| **4** | Approach 与原 Services 合并；删除 `What we run` 整屏；Work 前移并重组为旗舰案例与错层次级案例                      | 中   | 高   |
...
按 1440px 宽设计，窗口拉到 1440 以上看最准。可交互的部分：导航吸顶、Approach 滚动叙事、Work 数据流动画与案例图片 hover。页面底部有一条 `APPENDIX` 分隔线，线以下是设计系统、Header 对照和移动端稿，**不属于首页内容**。
