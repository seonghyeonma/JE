...
V4 不再把三个案例做成三条相同的横向列表，也不再用三张相同的卡片。三个案例内容不变，但建立明确主次：

```
[ PublicAI 大幅案例图 ]  [ 客户名 + 一句话结果 ]
[      560px 高       ]  [ scope + 三项纵向成果 ]

[ MOSS AI 宽图与成果 ]       [ SURF AI 较窄图与成果 ]
[ 先出现，承担主体重量 ]       [ 下移 136px，形成错层节奏 ]
```

PublicAI 是旗舰案例，使用约 `1.25 : 0.75` 的大图加数据叙事；MOSS AI 与 SURF AI 在下一层使用 `1.12 : 0.88` 的非对称双栏，SURF AI 主动下移形成作品索引的浏览节奏。移动端全部收回单栏，不保留错层。

案例编号贴片和 chip 全部删除。工作内容改为无边框的 scope 文本，避免案例区读起来像产品后台。hover 只让当前图片轻微放大和提亮，不再把其他案例整体压暗。
...
1. **数字继续用主色 + 等宽字**。旗舰案例使用 `40px`，次级案例使用 `33px`，通过层级而不是重复卡片建立重点。
...
4. **三项成果都与对应案例绑定**。页面先展示总体规模，再让三个案例分别回答这些结果是怎样发生的。

**区块开场：总体成绩概览**

聚合数据紧跟 `Work / What the system produced` 标题出现，再进入 PublicAI、MOSS AI、SURF AI 三个案例。顺序是“总成绩 → 具体证据”，因此图、数字和案例都属于同一个 Work 区块。

视觉上不再使用圆角四格卡片，也不再增加 `Impact at scale` 这层标签和独立章节标题。`Across every engagement to date` 只作为 Work 标题下的范围说明，下面直接是一张从收束到扩张的薄荷色数据流主视觉，以及 `100+` / `100M+` / `1,000+` / `15` 单行四列数据。删除模块外框和上下章节线，让它与 Work 标题形成同一个开场组合。
